...
export const examRoutes = async (app) => {
    app.post("/api/classes/:classId/exams", { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    app.get("/api/exams/:id", { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    app.post("/api/exams/:id/attempt", { preHandler: [requireAuth] }, async (request, reply) => {
        const { id } = request.params;
        ...
        if (!enrolled)
            return reply.status(403).send({ message: "Bukan peserta kelas" });
    ...
    app.post("/api/exams/:id/submit", { preHandler: [requireAuth] }, async (request, reply) => {
        const { id } = request.params;
        const exam = await prisma.exam.findUnique({ where: { id }, include: { class: { select: { teacherId: true } } } });
        if (!exam)
            return reply.status(404).send({ message: "Ujian tidak ada" });
        if (exam.class.teacherId === request.user.id) {
        ...
        if (!attempt)
            return reply.status(404).send({ message: "Belum memulai percobaan ujian (Attempt)" });
    ...
    app.post("/api/exams/:id/warning", { preHandler: [requireAuth] }, async (request, reply) => {
        const { id } = request.params;
        const exam = await prisma.exam.findUnique({ where: { id }, include: { class: { select: { teacherId: true } } } });
        if (!exam)
    ...
    app.get("/api/exams/:id/attempts/:studentId", { preHandler: [requireAuth] }, async (request, reply) => {
        ...
        if (!exam)
            return reply.status(404).send({ message: "Ujian tidak ada" });
        if (exam.class.teacherId !== request.user.id)
            return reply.status(403).send({ message: "Bukan kelas Anda" });
        const attempt = await prisma.examAttempt.findUnique({
    ...
    app.post("/api/exams/:id/attempts/:studentId/grade", { preHandler: [requireAuth] }, async (request, reply) => {
        const { id, studentId } = request.params;
    ...
    app.delete("/api/exams/:id", { preHandler: [requireAuth] }, async (request, reply) => {
        const { id } = request.params;
        const exam = await prisma.exam.findUnique({
            where: { id },
            include: { class: true },
